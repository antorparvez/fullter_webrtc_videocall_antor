...
// Adjust the join method to accept only the roomId argument
  Future<void> join(String appointmentId) async {
    _appointmentId = appointmentId;
    _localUuid = const Uuid().v1();

    if (_localStream == null) {
      throw Exception('You cannot start a call without the webcam opened');
    }

    // Check if the user has already joined the call
    if (_peerConnections.containsKey(_localUuid)) {
      if (kDebugMode) {
        print('User already joined the call');
      }
      // If user already joined, simply restart listeners
      _startListenSdp();
      _startListenIce();
      return;
    }
    ...
    final peers = await FirebaseFirestore.instance
        ...
        .where(
      'uuid',
      isNotEqualTo: _localUuid, // exclude myself
    )
        .get();
  ...
  Future<RTCPeerConnection> _createPeerConnection(
      String fromPeerId, String displayName) async {
    if (kDebugMode) {
    ...
    pc.onIceCandidate =
        (event) => _gotIceCandidate(event, fromPeerId, displayName);
  ...
  Future<void> _createdDescription(
      RTCPeerConnection pc,
      RTCSessionDescription description,
      String destinationPeerId,
      String sdpType) async {
    if (kDebugMode) {
  ...
  void _checkConnectionState(
      RTCIceConnectionState state, String peerUuid, String displayName) {
  ...
  void _gotRemoteStream(
      RTCTrackEvent event, String peerUuid, String displayName) {
    if (!_peerBanned.contains(peerUuid)) {
      if (kDebugMode) {
  ...
  Future<void> _gotIceCandidate(
      RTCIceCandidate iceCandidate, String peerUuid, String displayName) async {
    if (!_peerBanned.contains(peerUuid)) {
      if (kDebugMode) {
